...
| `/api/category` | POST | **`category`**, `page`, `pageSize`, **`subType`** | `CategoryResponse` | 分类+分页+子分类；后端 `totalPages` 不完全可信；共享后端会恢复磁盘热缓存、后台预热首屏，并在冷启动无缓存时先返回快速首屏 |
...
>
> ✅ v3.3 起 `/api/category` 增加服务端首屏热缓存：结果写入 `server/category_cache.json`，server 启动后恢复缓存并后台预热 `movie/tv/variety/anime/shortDrama/sports` 的移动端 20 条首屏和桌面端 30 条首屏。云端部署需要重新部署 server 后生效。
>
> ✅ v3.4 起 `/api/category` 冷启动无完整缓存时会先返回快速首屏：`complete:false`、`warming:true`、`list` 为主分类/高优先级 type_id 的临时结果；后端随后后台刷新完整聚合缓存。v3.5 起 Android 端只直接展示当前返回列表，不再客户端延迟静默刷新，避免后台请求和播放/详情跳转抢资源。
...
## 十三、v3.3 移动端/共享后端体验优化记录（2026-06-29）

### 13-A v3.5 移动端流畅度架构改造方案（2026-06-30）

移动端与桌面端共用 `server/server.js`，本地 IP 与云端测试均出现移动端明显卡顿、桌面端流畅，说明主要瓶颈在 Android 前端架构，而不是后端接口本身。

| 问题 | 当前表现 | 根因判断 |
|------|----------|----------|
| 分类分页慢 | 首屏/翻页等待明显，滚动触发加载不顺 | 移动端手写 `page++ + allItems`，同时存在全局分类预热、首页预热、分类页后续页预取，容易与当前用户请求争抢 OkHttp 连接/线程 |
| 封面进详情慢且偶发不匹配 | 点击 A 封面可能展示相近标题或其他站点内容 | 部分入口退化成 `title -> multi-detail` 名称搜索，缺少 `siteKey:vodId` 优先和标题/导演/年份二次校验 |
| 直连播放卡 | 桌面端同源流畅，移动端首帧慢/有画面但不播放 | Android 播放链路与普通 API/图片请求混用资源；ExoPlayer 使用默认缓冲策略，分享页/直链 MIME 判断和回退状态复杂 |
| 缓冲提示干扰 | 流畅播放时也会一会儿弹出 | UI 直接绑定 `onIsLoadingChanged`/`STATE_BUFFERING`，没有延迟阈值和 `isPlaying` 过滤 |

参考 GitHub 上成熟 Android 视频类项目的通用做法：CloudStream 将详情、播放链接解析、播放器拉流分层；NewPipe 这类轻量流媒体前端强调列表/详情/播放解耦；Android 官方 Paging 3 用分页数据源按需加载；Media3 通过 `DefaultLoadControl` 明确首帧缓冲、重缓冲和最大缓冲。

改造原则：

| 原则 | 说明 |
|------|------|
| 用户当前操作优先 | 点击详情、翻页、播放拉流的请求优先；首页/分类/详情的后台预热必须收敛，不能抢播放和当前页面加载 |
| Identity-first 详情匹配 | 有 `siteKey + vodId` 必须直拉详情；多源使用 `keys=site:id,...`；标题只做辅助校验，不作为首要身份 |
| 有限缓存 | 移动端只缓存最近分页/详情/搜索，不能把 3~4w 条分类塞进内存 |
| 播放链路独立 | 播放链接解析、ExoPlayer 拉流、普通 API/图片请求分离；播放器缓冲策略显式配置 |
| 缓冲 UI 延迟显示 | 只有 `!isPlaying` 且缓冲持续超过约 900ms 才展示提示；流畅播放中不弹层 |

执行顺序：

| 阶段 | 目标 | 改动 |
|------|------|------|
| Phase 1 | 立即降低卡顿 | 移除/暂停移动端全局预热、首页批量 warm detail、分类页后续页主动预取；保留服务端缓存 |
| Phase 2 | 修复详情不匹配 | 首页/分类/搜索入口优先走 `detail/{siteKey}/{vodId}`；多源只在 `sites` 有效时走 `detail/source/{name}?keys=`；详情页按标题归一化、导演、年份做严格校验 |
| Phase 3 | 分类加载架构 | 将 `CategoryScreen` 从手写分页迁移到 Paging 3 或等价的 ViewModel 分页状态机，UI 只消费当前页流 |
| Phase 4 | 播放器架构 | 引入播放器专用 LoadControl、缓冲 UI 延迟阈值、直链/代理状态简化；播放请求不与后台预取抢资源 |

### 13.0 共享后端 v3.4 补充（2026-06-29）

| 项目 | 状态 | 说明 |
|------|:--:|------|
| 分类冷启动快速首屏 | ✅ | `/api/category` 第 1 页无缓存且非子分类时，先拉各站主分类/高优先级 type_id，约 3 秒内返回 `complete:false` 临时结果 |
| 后台完整缓存刷新 | ✅ | 快速首屏返回后，server 后台继续执行完整父+子 type_id 聚合，完成后写入内存缓存和 `category_cache.json` |
| 移动端兼容 | ✅ | Android 直接展示临时 `list`；v3.5 起不再检测 `complete == false` 后延迟静默刷新，避免后台请求抢占资源 |

### 13.1 分类首屏与分页体验

| 项目 | 状态 | 说明 |
|------|:--:|------|
| 移动端分类加载 | ✅ | 分类首屏复用服务端快速首屏/热缓存；客户端不再主动预取后续页，只在滚动接近底部时按需加载下一页 |
| 稳定网格渲染 | ✅ | `LazyVerticalGrid` 增加稳定 key/contentType，底部 loading 跨整行，减少追加分页时布局抖动 |
| 有限缓存 | ✅ | Android 端仍只缓存最近 24 个分页，TTL 5 分钟，避免大分类全量进内存 |
| 服务端首屏热缓存 | ✅ | 共享后端新增 `category_cache.json` 磁盘缓存，server 启动恢复并后台预热移动/桌面分类首屏 |

### 13.2 播放线路修复

| 项目 | 状态 | 说明 |
|------|:--:|------|
| 分享页/HLS 判断 | ✅ | 无后缀分享页代理后继续强制 HLS MIME，保留 v2.9 的关键修复 |
| 直链兼容 | ✅ | 明确 `.mp4/.flv/.ts/.webm` 等代理播放不强制 HLS，交给 ExoPlayer 自动识别；失败后再 HLS 兜底 |

### 13.3 v3.5 移动端上线前体验收尾（2026-06-30）

| 项目 | 状态 | 说明 |
|------|:--:|------|
| 分类页最终策略 | ✅ | 删除客户端静默刷新、详情 warm、后续页预取等后台请求；保留首屏热缓存、分页有限缓存和滚动按需加载 |
| 封面异步加载 | ✅ | 分类/列表卡片先渲染文本骨架和布局，封面图异步补齐，避免等图片完成后才显示首屏 |
| 详情页进入过渡 | ✅ | 首页/分类/搜索入口先写入 `DetailPreviewCache`，详情页优先展示预览内容，再补全多源详情和播放源 |
| 详情页加载动画 | ✅ | 有预览数据时不再关闭 loading；完整详情/播放源请求未完成前显示局部补全动画，完全无预览时才显示全屏骨架 |
| 搜索资源合并 | ✅ | 搜索页参考桌面端使用 `/api/search-stream`，按归一化片名合并多个资源站为一张卡片，保留 `sites` |
| 播放线路简化 | ✅ | 移动端隐藏直连线路，只展示 m3u8 线路；详情页和播放页仍显示视频源名称，便于识别资源来源 |
| 收藏页容错 | ✅ | 收藏保存详情快照；“我的”页先读本地快照/占位，再后台刷新，远端失败不阻塞收藏列表展示 |
| 首页底部遮挡 | ✅ | 首页底部留白增加，避免底部导航栏遮挡“动漫”等最后一屏推荐卡片 |

---

## 十四、v3.2 移动端阶段收尾记录（2026-06-28）
...
> 📋 变更：
> - **v3.5** — 移动端上线前体验收尾：
>   - **分类页**：最终采用服务端首屏热缓存 + 客户端有限分页缓存 + 滚动按需加载；删除客户端延迟静默刷新、后续页主动预取和详情 warm，避免后台请求抢资源。
>   - **列表渲染**：封面图异步加载，先显示文本/骨架/布局，降低分类首屏等待感。
>   - **详情页**：入口写入预览缓存，详情页先展示预览内容，再补全多源详情和播放源；补全过程显示局部加载动画。
>   - **搜索页**：使用流式搜索并按片名归一化合并多源资源，移动端展示逻辑向桌面端靠齐。
>   - **播放线路**：移动端只显示 m3u8 线路；详情页/播放页保留视频源名称展示。
>   - **收藏/首页**：收藏页本地快照优先显示，远端后台刷新；首页底部留白修复导航栏遮挡。
> - **v3.4** — 共享后端分类冷启动快速首屏：
>   - **`/api/category`**：无完整缓存时先返回 `complete:false / warming:true` 的快速首屏结果，后台继续刷新完整父+子 type_id 聚合缓存。
>   - **移动端兼容**：当前可直接展示临时列表；v3.5 后 Android 不再客户端延迟静默刷新完整结果。
> - **v3.3** — 分类首屏与播放线路体验优化：
>   - **分类页**：稳定 key/contentType、loading 跨整行；后续页主动预加载方案已在 v3.5 撤销。
>   - **共享后端**：`/api/category` 增加 `category_cache.json` 磁盘热缓存，启动后恢复缓存并后台预热桌面/移动分类首屏。
>   - **播放器线路**：分享页/无后缀代理继续强制 HLS，明确 mp4/flv/ts 等直链不强制 HLS，失败后 HLS 兜底。
