...
>
> ✅ v3.4 起 `/api/category` 冷启动无完整缓存时会先返回快速首屏：`complete:false`、`warming:true`、`list` 为主分类/高优先级 type_id 的临时结果；后端随后后台刷新完整聚合缓存。Android 当前可直接展示 `list`，后续如要更丝滑可在 `complete == false` 时保留列表并延迟静默刷新一次。
...
| 移动端兼容 | ✅ | Android 可直接展示临时 `list`；如检测到 `complete == false`，后续可保留当前列表并延迟静默刷新完整结果 |

### 13.1 分类首屏与分页体验

| 项目 | 状态 | 说明 |
|------|:--:|------|
| 移动端分类预加载 | ✅ | 当前页加载完成后后台预取后续 2 页；接近底部提前约 4 行触发加载 |
...
>   - **移动端兼容**：当前可直接展示临时列表；后续如需进一步优化，可在 `complete == false` 时延迟静默刷新完整结果。
> - **v3.3** — 分类首屏与播放线路体验优化：
>   - **分类页**：移动端加入后续 2 页预加载、接近底部提前触发、稳定 key/contentType、loading 跨整行。
