...
**前端处理**：
- `CategoryPage` 收到 `complete:false` 时先展示列表，避免冷启动白屏。
- 桌面端可利用首页空闲期预解析/预热首屏；Android 端 v3.5 起不做客户端延迟静默刷新和后续页主动预取，避免后台请求抢占详情跳转和播放拉流资源。
- 移动端后续页只在滚动接近底部时按需加载；分类首屏主要依赖服务端内存缓存、`category_cache.json` 磁盘热缓存和 server 启动预热。
